            ...
            for (i = 0; i < data.length; i++) {
                for (j = 0; j < data[i].length; j++){
                    ...
                    for (cell_value = {}, l=0; l < self.data.measures.length; l++) {
                    	if (self.data.measures[l] === 'prc_cumpl_plan_monto') {
	                    	if (datapt['plan_monto'] === 0) {
		                        cell_value[self.data.measures[l]] = 0;
	                    	} else {
		                        cell_value[self.data.measures[l]] = datapt['real_monto'] / datapt['plan_monto'] * 100;
							}
                    	} else {
	                    	if (self.data.measures[l] === 'prc_convert_global') {
		                        if (i === 0) {
			                    	if (datapt['pipe_monto_anual'] === 0) {
				                        cell_value[self.data.measures[l]] = 0;
			                    	} else {
				                        cell_value[self.data.measures[l]] = datapt['real_monto'] / datapt['pipe_monto_anual'] * 100;
									}
			                    } else {
			                        if (i === 1) {
				                    	if (datapt['pipe_monto_trimestre'] === 0) {
					                        cell_value[self.data.measures[l]] = 0;
				                    	} else {
					                        cell_value[self.data.measures[l]] = datapt['real_monto'] / datapt['pipe_monto_trimestre'] * 100;
										}
				                    } else {
				                    	if (datapt['pipe_monto_mes'] === 0) {
					                        cell_value[self.data.measures[l]] = 0;
				                    	} else {
					                        cell_value[self.data.measures[l]] = datapt['real_monto'] / datapt['pipe_monto_mes'] * 100;
										}
									}
								}
	                    	} else {
		                    	if (self.data.measures[l] === 'prc_cumpl_plan_count') {
			                    	if (datapt['plan_count'] === 0) {
				                        cell_value[self.data.measures[l]] = 0;
			                    	} else {
				                        cell_value[self.data.measures[l]] = datapt['real_count'] / datapt['plan_count'] * 100;
									}
		                    	} else {
			                    	if (self.data.measures[l] === 'pipe_monto_prom_mes') {
				                        if (i === 0) {
				                        	cell_value[self.data.measures[l]] = datapt['pipe_monto_anual'];
					                    } else {
					                        if (i === 1) {
					                        	cell_value[self.data.measures[l]] = datapt['pipe_monto_trimestre'];
						                    } else {
						                        cell_value[self.data.measures[l]] = datapt['pipe_monto_mes'];
											}
										}
			                    	} else {
				                        if (self.data.measures[l] === 'pipe_monto_anual' || self.data.measures[l] === 'pipe_monto_trimestre' || self.data.measures[l] === 'pipe_monto_mes') {
				                        	l = l;
				                    	} else {
            ...
            for (var j = 0; j < colGroupBys.length + 1; j++) {
                for (var k = 0; k < data[index].length; k++) {
                    ...
                    for (cell_value = {}, m=0; m < this.data.measures.length; m++) {
                    	if (this.data.measures[m] === 'prc_cumpl_plan_monto') {
	                    	if (datapt['plan_monto'] === 0) {
		                        cell_value[this.data.measures[m]] = 0;
	                    	} else {
		                        cell_value[this.data.measures[m]] = datapt['real_monto'] / datapt['plan_monto'] * 100;
							}
                    	} else {
	                    	if (this.data.measures[m] === 'prc_convert_global') {
		                        if (i === 0) {
			                    	if (datapt['pipe_monto_anual'] === 0) {
				                        cell_value[this.data.measures[m]] = 0;
			                    	} else {
				                        cell_value[this.data.measures[m]] = datapt['real_monto'] / datapt['pipe_monto_anual'] * 100;
									}
			                    } else {
			                        if (i === 1) {
				                    	if (datapt['pipe_monto_trimestre'] === 0) {
					                        ...
					                        cell_value[this.data.measures[m]] = datapt['real_monto'] / datapt['pipe_monto_trimestre'] * 100;
										}
				                    } else {
				                    	if (datapt['pipe_monto_mes'] === 0) {
					                        cell_value[this.data.measures[m]] = 0;
				                    	} else {
					                        cell_value[this.data.measures[m]] = datapt['real_monto'] / datapt['pipe_monto_mes'] * 100;
										}
									}
								}
	                    	} else {
		                    	if (this.data.measures[m] === 'prc_cumpl_plan_count') {
			                    	if (datapt['plan_count'] === 0) {
				                        cell_value[this.data.measures[m]] = 0;
			                    	} else {
				                        cell_value[this.data.measures[m]] = datapt['real_count'] / datapt['plan_count'] * 100;
									}
		                    	} else {
			                    	if (this.data.measures[m] === 'pipe_monto_prom_mes') {
				                        if (i === 0) {
				                        	cell_value[this.data.measures[m]] = datapt['pipe_monto_anual'];
					                    } else {
					                        if (i === 1) {
					                        	cell_value[this.data.measures[m]] = datapt['pipe_monto_trimestre'];
						                    } else {
						                        cell_value[this.data.measures[m]] = datapt['pipe_monto_mes'];
											}
										}
			                    	} else {
				                        if (this.data.measures[m] === 'pipe_monto_anual' || this.data.measures[m] === 'pipe_monto_trimestre' || this.data.measures[m] === 'pipe_monto_mes') {
				                        	m = m;
				                    	} else {
